function outputData = createCaptureOutput(s,fscvparam)
%Based on fscvparam and session s parameters, create analog output signals:
%fscv ramp, stim output, and custom clock, for actual recording
...
stimType=fscvparam.stimType;        %1 is mono, 2 is bi

lagbetweenIO=500;

output_length = max(round(s.Rate*timeToRecord), 15*s.Rate);     %needs to be at least as long as record period so stim does not repeat

%fscv triangle ramp waveform
f_cycle_samples=freq./(s.Rate);
V_amp=1;
ramp_factor=1;
